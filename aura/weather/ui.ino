// UI construction, input reading, and event handling
  ...
  if (touchscreen.tirqTouched() && touchscreen.touched()) {
    ...
    if (night_mode_active) {
      if (!temp_screen_wakeup_active) {
        // Wake-up tap: undim the screen but don't pass the touch to the UI
        temp_screen_wakeup_active = true;
        analogWrite(LCD_BACKLIGHT_PIN, display_brightness);
        temp_screen_wakeup_timer = lv_timer_create(handle_temp_screen_wakeup_timeout, TEMP_WAKEUP_TIMEOUT_MS, NULL);
        lv_timer_set_repeat_count(temp_screen_wakeup_timer, 1);  // Run only once
        Serial.println("Woke up screen. It will dim again after 15 seconds of inactivity.");
        data->state = LV_INDEV_STATE_RELEASED;
        return;
      }

      // Screen already awake: keep postponing the dim while the user interacts
      if (temp_screen_wakeup_timer) {
        lv_timer_reset(temp_screen_wakeup_timer);
      }
    }
...
void wifi_splash_screen() {
  ...
  lv_obj_set_style_text_font(lbl, &lv_font_montserrat_latin_14, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void create_ui() {
  ...
  lv_obj_set_style_text_font(lbl_today_temp, &lv_font_montserrat_latin_42, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_today_feels_like, &lv_font_montserrat_latin_14, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_forecast, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  for (int i = 0; i < 7; i++) {
    ...
    lv_obj_set_style_text_font(lbl_daily_day[i], &lv_font_montserrat_latin_16, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_font(lbl_daily_high[i], &lv_font_montserrat_latin_16, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_font(lbl_daily_low[i], &lv_font_montserrat_latin_16, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  for (int i = 0; i < 7; i++) {
    ...
    lv_obj_set_style_text_font(lbl_hourly[i], &lv_font_montserrat_latin_16, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_font(lbl_hourly_temp[i], &lv_font_montserrat_latin_16, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_font(lbl_precipitation_probability[i], &lv_font_montserrat_latin_16, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_clock, &lv_font_montserrat_latin_14, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void populate_results_dropdown() {
  ...
  if (geoResults.size() > 0) {
    lv_dropdown_set_options(results_dd, dd_opts.c_str());
...
static void reset_wifi_event_handler(lv_event_t *e) {
  ...
  lv_obj_set_style_text_font(title, &lv_font_montserrat_latin_16, 0);

  lv_obj_t *text = lv_msgbox_add_text(mbox, strings->reset_confirmation);
  lv_obj_set_style_text_font(text, &lv_font_montserrat_latin_12, 0);
  lv_msgbox_add_close_button(mbox);

  lv_obj_t *btn_no = lv_msgbox_add_footer_button(mbox, strings->cancel);
  lv_obj_set_style_text_font(btn_no, &lv_font_montserrat_latin_12, 0);
  lv_obj_t *btn_yes = lv_msgbox_add_footer_button(mbox, strings->reset);
  lv_obj_set_style_text_font(btn_yes, &lv_font_montserrat_latin_12, 0);
...
void create_location_dialog() {
  ...
  lv_obj_set_style_text_font(lbl2, &lv_font_montserrat_latin_14, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(results_dd, &lv_font_montserrat_latin_14, LV_PART_MAIN | LV_STATE_DEFAULT);
  lv_obj_set_style_text_font(results_dd, &lv_font_montserrat_latin_14, LV_PART_SELECTED | LV_STATE_DEFAULT);

  lv_obj_t *list = lv_dropdown_get_list(results_dd);
  lv_obj_set_style_text_font(list, &lv_font_montserrat_latin_14, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_close, &lv_font_montserrat_latin_14, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_cancel, &lv_font_montserrat_latin_14, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void create_settings_window() {
  ...
  lv_obj_set_style_text_font(lbl_b, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
  lv_obj_align(lbl_b, LV_ALIGN_TOP_LEFT, 0, 5);
  lv_obj_t *slider = lv_slider_create(cont);
  lv_slider_set_range(slider, 1, 255);
  lv_slider_set_value(slider, display_brightness, LV_ANIM_OFF);
  lv_obj_set_width(slider, 100);
  lv_obj_align_to(slider, lbl_b, LV_ALIGN_OUT_RIGHT_MID, 10, 0);

  // Apply brightness live while dragging, but only persist to NVS on release
  lv_obj_add_event_cb(slider, [](lv_event_t *e){
    lv_obj_t *s = (lv_obj_t*)lv_event_get_target(e);
    display_brightness = lv_slider_get_value(s);
    analogWrite(LCD_BACKLIGHT_PIN, display_brightness);
  }, LV_EVENT_VALUE_CHANGED, NULL);
  lv_obj_add_event_cb(slider, [](lv_event_t *e){
    prefs.putUInt("brightness", display_brightness);
  }, LV_EVENT_RELEASED, NULL);

  // 'Night mode' switch
  lv_obj_t *lbl_night_mode = lv_label_create(cont);
  lv_label_set_text(lbl_night_mode, strings->use_night_mode);
  lv_obj_set_style_text_font(lbl_night_mode, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_u, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_24hr, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_loc, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_lang, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(language_dropdown, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
  lv_obj_set_style_text_font(language_dropdown, &lv_font_montserrat_latin_12, LV_PART_SELECTED | LV_STATE_DEFAULT);
  lv_obj_t *list = lv_dropdown_get_list(language_dropdown);
  lv_obj_set_style_text_font(list, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_chg, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_reset, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(lbl_btn, &lv_font_montserrat_latin_12, LV_PART_MAIN | LV_STATE_DEFAULT);
...
static void settings_event_handler(lv_event_t *e) {
  ...
  if (tgt == language_dropdown && code == LV_EVENT_VALUE_CHANGED) {
    ...
    // child of the screen, including the keyboard and any open location dialog,
    // so null the pointers to avoid reusing freed objects later.
